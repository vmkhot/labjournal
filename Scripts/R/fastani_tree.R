...
dend_2 <- dend %>% set("labels_cex",0.6) %>% set("branches_lwd",0.5) %>% set("hang_leaves")

# tree <- as.phylo(h)
# write.tree(phy=tree, file='fastani.tree')
...
p2 <-ggplot(C,aes(y=sqrt(C$values),x=C$contigName,fill=factor(C$samples))) +
  ...
  theme(axis.title.y = element_blank(),plot.title=element_text(hjust=0.5, size=11),legend.text=element_text(size=12),legend.title=element_text(size=12))+
...
plot_grid(p1,p2,align="h",scale = c(1.025,1),rel_widths = c(1.6,1))
